...
If you did not save your MaaS model endpointURL, navigate back to the 3Scale developer portal to grab it:
...
* In case you are not in the specific project, earch for the `lls-demo` namespace
...
* Enable the OpenShift MCP and Slack MCP tool groups.
...
[source,console,role=execute,subs=attributes+]
----
Get logs for the slack-mcp-server pod.
----
...
[source,console,role=execute,subs=attributes+]
----
Post the slack-mcp-server pod logs to the #all-summit-connect-2025 channel. Add "Hi from <insert-event-city-name>" to the beginning of the message.
----
...
This helps to filter inappropriate prompts and responses. Test it out!
